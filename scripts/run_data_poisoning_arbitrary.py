import sys
sys.path.append('/Users/matthewjagielski/Documents/influence/influence-release')

import IPython
import numpy as np

from load_animals import load_animals, load_dogfish_with_koda, load_dogfish_with_orig_and_koda, load_dogfish_with_pois
...
assert argv[1] in ['subpop', 'koda', 'rand']
which_dataset = (argv[1], int(argv[2]))

if which_dataset[0] == 'koda':
    dataset_name = 'dogfish_koda'
    data_sets = load_dogfish_with_pois(koda=True)
elif which_dataset[0] == 'rand':
    dataset_name = 'dogfish_rand'+str(which_dataset[1])
    data_sets = load_dogfish_with_pois(random=which_dataset[1])
else:  #  which_dataset[0] == 'subpop':
    dataset_name = 'dogfish_subpop'+str(which_dataset[1])
    data_sets = load_dogfish_with_pois(subpop=which_dataset[1])
